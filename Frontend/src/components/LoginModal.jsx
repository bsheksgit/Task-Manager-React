...
export default function LoginModal() {
    ...
    const loginFailed = useSelector(state => state.loginModal.loginModal.loginFailed);
    ...
    async function handleLogin(e) {
        ...
        try {
            const response = await sendRequest(apiHelper.loginUser, {email, password});
            console.log('Login response:', response);
            if (response.error) {
                console.error('Login error:', response.error);
                dispatch(commonActions.openSnackbar({ message: 'Login failed. '+ (response.error.detail || 'Please check your credentials and try again.') }));
                dispatch(loginModalActions.loginFailure(response.error.detail || 'Login failed'));
                return;
            }
            ...
            if (response.token) {
                ...
                dispatch(loginModalActions.loginSuccess({
            ...
            dispatch(commonActions.openSnackbar({ message: response.message || 'Login successful!' }));
        }
        catch (error) {
            console.error('Error during login:', error);
            dispatch(commonActions.openSnackbar({ message: 'Login failed. Please try again.' }));
    ...
    return (
        ...
        <h1 className="text-4xl font-bold text-gray-800">Enter your credentials</h1>
